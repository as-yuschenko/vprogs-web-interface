<?php
	...
	try
	{
		$db->exec('UPDATE ppRelay SET mainScreen = 0');
	}
	catch (Exception $e) 
	{
	...
	$stmt_mScr = $db->prepare('UPDATE ppRelay SET mainScreen = 1 where id=:id');
	//prepare desc query
	$stmt_desc = $db->prepare('UPDATE ppRelay SET desc=:desc where id=:id');
